    ...
    $.form = new function (that) {
        ...
        this.href = function (url, obj) {
            ...
            if (url !== '#') {
                 ...
                 *
                 * Change the anchor part by setting the location.hash or location.href
                 * property of the Location Object
                 *
                 * 重复同一地址,不会触发onhashchange事件唷！！！ 优雅(๑•̀ㅂ•́)و✧
                 */
                window.location.href = '#' + $.menu.parseUri(url, obj);
            } else if (obj && obj.getAttribute('data-menu-node')) {
                /**
                 * 触发【顶级节点菜单开头的第一个三级节点】点击事件
                 * 例:
                 *   顶级节点:【商城管理】【m-13】
                 *   二级节点:【商城配置】【m-13-47】(若二级节点没有子节点,则触发二级节点)
                 *           【数据管理】【m-13-48】
                 *   三级节点:【商城参数配置】【m-13-47-45】
                 *           【短信发送记录】【m-13-47-46】
                 */
            ...
            window.onhashchange = function (hash, node) {
                ...
                if (/^m-/.test(node)) {
                    ...
                    if (node.split('-').length > 2) {
                         ...
                         * }
                         */
                        $('.layui-layout-admin').removeClass('layui-layout-left-hide');  // 左侧菜单显示

                    } else $('.layui-layout-admin').addClass('layui-layout-left-hide');  // 左侧菜单隐藏
