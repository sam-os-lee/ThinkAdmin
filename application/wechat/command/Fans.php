...
class Fans extends Command
{
    ...
    protected function _list($next = '', $index = 0)
    {
        ...
        while (true) {
            if (is_array($result = $wechat->getUserList($next)) && !empty($result['data']['openid'])) {
                ...
                if (in_array($result['next_openid'], $result['data']['openid'])) {
                    break;
                }
    ...
    public function _black($next = '', $index = 0)
    {
        ...
        while (true) {
            if (is_array($result = $wechat->getBlackList($next)) && !empty($result['data']['openid'])) {
                foreach (array_chunk($result['data']['openid'], 100) as $chunk) {
