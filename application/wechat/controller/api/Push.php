...
class Push extends Controller
{
    ...
    private function keys($rule, $isLast = false, $isCustom = false)
    {
        ...
        switch (strtolower($data['type'])) {
            ...
            case 'image':
                if (empty($data['image_url']) || !($mediaId = MediaService::upload($data['image_url'], 'image'))) {
                    return false;
                }
            ...
            case 'video':
                ...
                if (!($mediaId = MediaService::upload($data['video_url'], 'video', $videoData))) {
                    return false;
                }
    ...
    private function sendMessage($type, $data, $isCustom = false)
    {
        if ($isCustom) {
            ...
            case 'customservice': // 转交客服消息
                if ($data['content']) {
                    $this->sendMessage('text', $data, true);
                }
