...
class Config extends Controller
{
    ...
    public function options()
    {
        ...
        if ($this->request->isGet()) {
            ...
            $this->authurl = config('wechat.service_url') . "/service/api.push/auth/{$code}";  // 消息推送api
    ...
    public function payment()
    {
        ...
        if ($this->request->isGet()) {
            ...
            if (!$file->has($this->wechat_mch_ssl_cer, true)) {
                $this->wechat_mch_ssl_cer = '';
            }

            if (!$file->has($this->wechat_mch_ssl_key, true)) {
                $this->wechat_mch_ssl_key = '';
            }

            if (!$file->has($this->wechat_mch_ssl_p12, true)) {
